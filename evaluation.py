...
logging.basicConfig(level=logging.DEBUG,  # 控制台打印的日志级别
                    filename='log/evaluation-' + timemark + '.log',
                    filemode='w',
                    format=
...
class KeyphraseEvaluator:
    def __init__(self, top_n, metrics_mode, token_field='tokens', true_keyphrase_field='keyphrases',
    ...
    def evaluate_macro_average(self, data_source, eval_mode):
        self.__check_eval_mode(eval_mode)  # 检查评价模式是否合法
        top_counter = OrderedDict()
        for n in self.top_n:
            counter = []
            for record in data_source:
