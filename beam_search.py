...
from dataloader import (TOKENS, TOKENS_LENS, TOKENS_OOV, OOV_COUNT, OOV_LIST, EOS_WORD, UNK_WORD)
import logging
import time

timemark = time.strftime('%Y%m%d-%H%M%S', time.localtime(time.time()))
logger = get_logger('beam_search')
fh = logging.FileHandler('log/【beam search】' + timemark + '.log')
...
class BeamSearch:
    ...
    def beam_search(self, src_dict, delimiter=None):
        ...
        # decoder_prob: B * (vocab_size * src_size)
        # decoder_state: B * (hidden*2 + embed_size)
        # hidden_state: 1 * B * hidden_size
        prev_best_probs, prev_best_index = torch.topk(decoder_prob, self.beam_size, 1)
        # oov --> unk
        oov_token_mask = prev_best_index >= self.vocab_size  # 定位oov在哪
        prev_best_index.masked_fill_(oov_token_mask, self.unk_idx)
        prev_best_index.masked_fill_(oov_token_mask, self.unk_idx)
        # hidden_state[0]: B * hidden_size
        ...
        hidden_state = hidden_state.unsqueeze(2).repeat(1, 1, self.beam_size, 1)
        hidden_state = hidden_state.view(-1, beam_batch_size, hidden_state.size(-1))
        prev_hidden_state = hidden_state
        ...
        for target_idx in range(1, self.max_target_len):
            ...
            prev_hidden_state = prev_hidden_state.index_select(1, state_select_idx)
    ...
    def expand_encoder_output(self, encoder_output_dict, batch_size):
        ...
        encoder_hidden_state = encoder_hidden_state.unsqueeze(2).repeat(1, 1, self.beam_size, 1)
        encoder_hidden_state = encoder_hidden_state.reshape(-1, beam_batch_size, hidden_size)
